...
rm(artificial_controls_metadata, barcode_counts_aggregated, barcode_metadata, counts, samples, pheno_data)
```
...
- Evaluate absolute thresholds on total read counts

```{r}
test_filtered <- bartools::thresholdCounts(
  dge_count,
  type = "relative",
  threshold = 1e-5,
  minSamples = 3,
...
ggsave("./figures/barcode_filtering/test_noise_filtering_plot.pdf", 
       test_filtered, dpi = 600, width = 14, height = 14)
```
