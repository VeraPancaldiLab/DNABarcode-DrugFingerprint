...
exp200921_expression <- readr::read_csv("data/barcode-counts/exp200921.csv") 
...
[While the *dose-response* experience `exp200921_dose response osim` exhibits distinct biological objective, it turned out that the 4 control biological replicates are shared in both experiences. However, **the number of sequenced barcode IDs differ in both experiences.**.]{fg="red"}
...
In @tbl-overview-barcode-samples, we list the final selection of `{r} ncol(barcode_summaries)` barcode experiences considered so far in the analysis.

```{r}
#| label: tbl-overview-barcode-samples
#| tbl-cap: "Overview of the 11 Barcode experiences preserved in the pipeline."
...
pheno_data <- pheno_data |> 
  ...
  dplyr::mutate(Batch_ID = dplyr::if_else(Date=="200921" & Samples == "Osimertinb" & `Run date` =="101121", 
...
In Appendix @sec-ID-mapping, we report a semi-automated approach to ensure proper mapping between shortID notation used in barcode counts matrices, and their respective full `Samples` ID peers in `Table of Compounds`.

Best next time: ensure a 1-1 mapping with full `Samples` names^[`Dorsomorphin (Compound C) 2HCl` to shorter `Samples_ID:Compou` was especially hard to infer].

[**Please ensure now that the *Sheet* `Drugs Mapping` in [`Table of compounds`](data/Table of compounds.xlsx) is correct.**]{fg="red"}
...
<!-- #|   - "Anti-join between *counts* and *phenotype*, returning counts counts that could not have been mapped back to `Table of Compounds`." -->
...
pheno_data_formatted <- pheno_data |> dplyr::inner_join(ID_mapping,
                                                        by = c("Batch_ID", "Date", 
                                                               "Samples", "Run date",
...
3.  **Derivation of average basal barcoding counts** (in controls, in other words)

  -   [Calculate mean of the 4 controls]{fg="blue"}
  -   [Batch effect correction for integrating across batches?]{fg="red"}.
...
5.  **Derivation of drug fingerprints**:
  ...
  -   [Calculate the binarised sum of the four replicates per drug]{fg="blue"}
  -   [Efficient computation of binary values with a logical `AND` + perform **sensitivity analyses** to evaluate the impact of threshold variation.]{fg="red"}.

## Correlation across Samples

1.  **Global perturbagen profile derivation**:

-   [Merge all barcode counts profiles keeping only the most abundant?]{fg="blue"}
...
2.  **Drug correlation**:
    i. Actual computation of the correlation matrix using `stats::cor`. [Yet, we have to try other correlation methods on the continuous space, or consider other metrics if working on the discrete space.]{fg="red"}
    ii. Plot weighted undirected graphs with `igraph::graph_from_adjacency_matrix`
    
3.  **Barcode correlation**: [To select barcodes highly correlated within each other, must correlate, with $CC > 0.8$ with at least 4 others.]{fg="blue"}.
...
- Tools:
  - `GitHub` repository on Vera's organisation
