...
row.names(barcode_metadata) <- barcode_counts_normalised$barcode_id
...
barcode_df_compound_by_batch <- tibble::tibble(original =setdiff(colnames(barcode_counts_wider_compound_by_batch),"barcode_id")) |>
...
se_compound_by_batch <- SummarizedExperiment::SummarizedExperiment(
  assays = list(counts = barcode_counts_wider_compound_by_batch |> 
                  ...
                  as.matrix()),
  rowData = barcode_metadata,
  colData = barcode_df_compound_by_batch)
...
barcode_df_compound <- tibble::tibble(Compound = setdiff(colnames(barcode_counts_wider_by_compound), "barcode_id")) |>
...
### **Derivation of compound fingerprints**:

- [Aggregate by compound and experience.]{fg="blue"} 

```{r}
#| label: compound-fingerprint-estimation
#| lst-label: lst-compound-fingerprint-estimation
#| lst-cap: "Aggregate at the compound level, assigning a 1 if, and only if, barcode cell lines were found significant in a given compound."

# 1) discretised correlation at the replicate level
barcode_discretised_replicates <- barcode_counts_discretised |> 
...
saveRDS(barcode_discretised_replicates, 
        file="results/fingerprints_replicates_level_", today_date, ".rds")

# 2) discretised correlation at the compound by batch level (averaging over replicates)
barcode_counts_discretised_compound_by_batch <- barcode_counts_discretised |> 
...
barcode_counts_discretised_compound_by_batch <- barcode_counts_discretised_compound_by_batch |> 
  ...
  tidyr::pivot_wider(id_cols = barcode_id, names_from = Compound, values_from = Barcode_Counts)
saveRDS(barcode_counts_discretised_compound_by_batch, 
        file=paste0("results/fingerprints_compounds_by_batch_level_", today_date, ".rds"))

# 3) discretised correlation at the compound level (averaging over all batches, concentrations and time courses)
...
barcode_discretised_replicates_mat <- barcode_counts_discretised_replicates |> 
  tibble::column_to_rownames(var="barcode_id") |> 
  as.matrix()
cor_replicates <- cor(barcode_discretised_replicates_mat)


barcode_discretised_compounds_mat <- barcode_counts_discretised_compound |> 
  tibble::column_to_rownames(var="barcode_id") |> 
  as.matrix()
cor_compounds <- cor(barcode_discretised_compounds_mat)
```
...
#### Heatmaps using only the global Pearson correlation

- In @fig-heatmap-global-replicate, it's generated at the replicate level, with a matrix of `{r} nrow(barcode_counts_mat)` by `{r} ncol(barcode_counts_mat)`
...
se_normalised <- readRDS("./results/normalised_counts/se_normalised_2025-04-28.rds")
...
col_cor_scale <- circlize::colorRamp2(c(min(cor_per_replicate), 0, 1),
                                      c("blue", "white", "red"))
...
# Define global Heatmap
heatmap_global <- ComplexHeatmap::Heatmap(cor_per_replicate, 
                                          col=col_cor_scale, 
...
ggsave(paste0("figures/heatmap_global_replicates_", today_date,".pdf"), 
       heatmap_global, dpi = 600, width = 20, height = 20)
```

#### Heatmap from binarised values

- Heatmap at the compound and replicate level are reported in @fig-generate-heatmap-compound.

```{r}
#| label: fig-generate-heatmap-compound
#| fig-cap: "Heamaps at the compound and replicate level."
heatmap_compounds <- ComplexHeatmap::Heatmap(cor_compounds, 
                                         col=col_cor_scale, 
                                         name = "CC at Compound and Batch Level")
heatmap_replicates <- ComplexHeatmap::Heatmap(cor_replicates, 
                                              col=col_cor_scale, 
                                              name = "CC at Individual Replicate Level")
