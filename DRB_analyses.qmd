...
pheno_data <- pheno_data |> 
  ...
  dplyr::mutate(Batch_ID = dplyr::if_else(Comments %in% c("Time Course"), 
                ...
                Level1 = if_else(is.na(Level1), Level2, Level1)) |> 
...
<!-- - [For compound `SBI-0206965`, both `Date` and `Run date` were wrongly reported (either not stored in the proper barcode counts profile, or mistyped in `Table of Compounds`).]{fg="red"} -->
...
**Conclusion**: on a total of `{r} nrow(ID_mapping)`, `{r} nrow(pheno_data_formatted)` replicates are kept, and `{r} nrow(replicates_discarded)` replicates were removed^[**Side note**: there are `{r} length(unique(pheno_data_formatted$Replicates_ID))` replicates, less than the total number of rows, due to the 4 Controls shared between `exp200921` and `exp200921_dose response osim`.].
...
- After filtering for background noise, and merging all experiences based on shared barcode IDs, `{r} nrow(barcode_counts_aggregated)` unique barcode IDs are kept, for `{r} rnow(pheno_data)` samples, see @lst-background-filtering for details.
...
barcode_counts_wider_compound_by_batch <- barcode_counts_long |> 
...
pheno_compound_by_batch <- tibble::tibble(original =setdiff(colnames(barcode_counts_wider_compound_by_batch),"barcode_id")) |>
...
se_compound_by_batch <- SummarizedExperiment::SummarizedExperiment(
  assays = list(counts = barcode_counts_wider_compound_by_batch |> 
                  tibble::column_to_rownames("barcode_id") |> 
                  as.matrix()),
  rowData = barcode_metadata,
  colData = pheno_compound_by_batch)
...
saveRDS(se_replicates, file = paste0("./results/compounds/se_normalised_per_replicate_",
                          today_date, ".rds"))
saveRDS(se_compound_by_batch, file = paste0("./results/compounds/se_normalised_per_compound_by_batch_",
                          today_date, ".rds"))
...
1. [Calculate mean of the 4 controls, followed by **Differential Represented Barcode Analysis**: binarise each replicate, assigning a 1 if Fold change is above 3 with respect to Mean value, and 0 otherwise^[The `Warning: Detected an unexpected many-to-many relationship between x and y.` results from shared Control samples.]]{fg="blue"} 
...
barcode_discretised_compound_by_batch <- barcode_discretised_replicates |> 
   tidyr::pivot_wider(id_cols = c(barcode_id), 
                     names_from = c(Batch_ID, Compound, Concentrations_ID, Duration_ID),
                     names_sep = ":",
                     values_from = Barcode_Counts, 
...
pheno_discretised_compound_by_batch <- pheno_compound_by_batch[setdiff(colnames(barcode_discretised_compound_by_batch), "barcode_id"), ]

se_compound_by_batch_discretised <- SummarizedExperiment::SummarizedExperiment(
  assays = list(discretised = barcode_discretised_compound_by_batch |> 
                  tibble::column_to_rownames("barcode_id") |> 
                  as.matrix()),
  rowData = barcode_metadata,
  colData = pheno_discretised_compound_by_batch)

saveRDS(se_compound_by_batch_discretised,
...
- Heatmap in @fig-heatmap-global-replicate is generated at the replicate level:
...
se_normalised <- readRDS("./results/compounds/se_normalised_per_replicate_2025-04-28.rds")
se_normalised_mat <- SummarizedExperiment::assay(se_normalised)
se_pheno_data <- SummarizedExperiment::colData(se_normalised)
se_pheno_data$log10Conc <- if_else(se_pheno_data$Concentrations==0, 
                                   0, 
                                   -log10(se_pheno_data$Concentrations))
# Compute global correlation score
cor_per_replicate <- cor(se_normalised_mat)
...
num_batches <- length(unique(se_pheno_data$Batch_ID))
num_compounds <- length(unique(se_pheno_data$Compound))
num_moa <- length(unique(se_pheno_data$MoA))

batch_col <- setNames(colorRampPalette(brewer.pal(8, "Dark2"))(num_batches), 
                      unique(se_pheno_data$Batch_ID))
compound_col <- setNames(colorRampPalette(brewer.pal(11, "Spectral"))(num_compounds), 
                         unique(se_pheno_data$Compound))
moa_col <- setNames(colorRampPalette(brewer.pal(12, "Set3"))(num_moa), 
                    unique(se_pheno_data$MoA))


## Heatmap annotation
top_ha <- ComplexHeatmap::HeatmapAnnotation(df = se_pheno_data |> 
                                          as.data.frame() |> 
...
row_ha <- ComplexHeatmap::rowAnnotation(Concentration =
                                          ComplexHeatmap::anno_barplot(se_pheno_data$log10Conc), 
                                        Duration = paste0(se_pheno_data$Duration, "d"))
...
```{r}
#| label: fig-heatmap-normalised-replicate
#| fig-cap: "Heatmap of compounds by batch(aggregated over replicates) with CC."

# Read SummarizedExperiment object
se_normalised <- readRDS("./results/compounds/se_discretised_per_compound_by_batch_2025-04-28.rds")
se_normalised_mat <- SummarizedExperiment::assay(se_normalised)
se_pheno_data <- SummarizedExperiment::colData(se_normalised)
# Compute global correlation score
cor_per_compound_by_batch <- cor(se_normalised_mat)
...
## Heatmap annotation
ha <- ComplexHeatmap::HeatmapAnnotation(df = se_pheno_data |> 
                                          as.data.frame() |> 
                                          dplyr::select(Pathway))

# Define Heatmap
heatmap_normalised_compound_by_batch <- ComplexHeatmap::Heatmap(cor_per_compound_by_batch, 
                                          col=col_heatmap_scale, 
                                          name = "CC for all Compounds", 
                                          show_row_names = TRUE,
...
ggsave(paste0("figures/compounds/heatmap_normalised_compound_by_batch_", today_date,".pdf"), 
       heatmap_normalised_compound_by_batch, dpi = 600, width = 20, height = 20)
```
...
# Read SummarizedExperiment object
se_normalised <- readRDS("./results/compounds/se_normalised_per_compound_2025-04-28.rds")
se_normalised_mat <- SummarizedExperiment::assay(se_normalised)
se_pheno_data <- SummarizedExperiment::colData(se_normalised)
# Compute global correlation score
cor_per_compound <- cor(se_normalised_mat)
...
- Heatmap at the replicates level 
...
# Read SummarizedExperiment object
se_binarised <- readRDS("./results/compounds/se_discretised_per_replicate_2025-04-28.rds")
se_binarised_mat <- SummarizedExperiment::assay(se_binarised)
se_pheno_data <- SummarizedExperiment::colData(se_binarised)
# Compute global correlation score
cor_per_replicates <- cor(se_binarised_mat)
...
# Read SummarizedExperiment object
se_binarised <- readRDS("./results/compounds/se_discretised_per_compound_by_batch_2025-04-28.rds")
se_binarised_mat <- SummarizedExperiment::assay(se_binarised)
se_pheno_data <- SummarizedExperiment::colData(se_binarised)
# Compute global correlation score
cor_per_compound_by_batch <- cor(se_binarised_mat)
...
heatmap_binarised_compound_by_batch <- ComplexHeatmap::Heatmap(cor_per_compound_by_batch, 
                                            col=col_heatmap_scale, 
                                            name = "CC at binarised Compound Level",
                                            show_row_names = TRUE,
                                            show_column_names = TRUE, 
...
ggsave(paste0("figures/compounds/heatmap_binarised_compound_by_batch_", today_date,".pdf"), 
       heatmap_binarised_compound_by_batch, dpi = 600, width = 20, height = 20)                                            
```
...
# Read SummarizedExperiment object
se_binarised <- readRDS("./results/compounds/se_discretised_per_compound_2025-04-28.rds")
se_binarised_mat <- SummarizedExperiment::assay(se_binarised)
se_pheno_data <- SummarizedExperiment::colData(se_binarised)
# Compute global correlation score
cor_per_compound <- cor(se_binarised_mat)
...
#| fig-cap: " at the compound level."

# 1) Compute correlation matrix and p-values ----
cor_res <- Hmisc::rcorr(se_binarised_mat, type = "pearson")
...
vertex_structure <- se_pheno_data |> 
  as.data.frame() |> 
...
# Read SummarizedExperiment object
se_binarised_compound_by_batch <- readRDS("./results/compounds/se_discretised_per_compound_by_batch_2025-04-28.rds")
se_binarised_compound_by_batch_mat <- SummarizedExperiment::assay(se_binarised_compound_by_batch) |> 
  as.matrix() 
se_binarised_compound_by_batch_fac <- apply(se_binarised_compound_by_batch_mat, 2, as.character)
se_pheno_data_compound_by_batch <- SummarizedExperiment::colData(se_binarised_compound_by_batch)

## Heatmap annotation
ha <- ComplexHeatmap::HeatmapAnnotation(df = se_pheno_data_compound_by_batch |> 
                                          as.data.frame() |> 
                                          dplyr::select(Batch_ID, Pathway, Compound))

heatmap_cell_lines_by_sample <- ComplexHeatmap::Heatmap(se_binarised_compound_by_batch_fac, 
...
present_cells <- rowSums(se_binarised_compound_by_batch_mat)
hist(present_cells, 
...
se_binarised_compact <- se_binarised_compound_by_batch_mat[present_cells >
