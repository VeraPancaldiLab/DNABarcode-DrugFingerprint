...
### Save `SummarizedExperiment` at different levels of granularity
...
#| lst-cap: Simpy compute the Pearson correlation score at the replicate level, and save the output as a `SummarizedExperiment`.

# 2) prepare SummarizedExperiment inputs ----
barcode_counts_mat <- barcode_counts_normalised |> 
...
# Feature metadata
barcode_df <- tibble::tibble(barcode_ID <- barcode_counts_normalised$barcode_id)
row.names(barcode_df) <- barcode_counts_normalised$barcode_id

# 3) Buid and Save SummarizedExperiment
se <- SummarizedExperiment::SummarizedExperiment(
  assays = list(counts = barcode_counts_mat),
  rowData = barcode_df,
  colData = pheno_data_df)

saveRDS(se, file = paste0("./results/SummarizedExperiment/se_normalised_per_replicate_",
                          today_date, ".rds"))
# rm(barcode_counts_mat, pheno_data_df, barcode_df, se)
``` 

- Medium level of granularity in @lst-SummarizedExperiment-compound-per-batch:
...
barcode_counts_wider_drug_by_batch <- barcode_counts_long |> 
...
barcode_df_drug_by_batch <- tibble::tibble(original = 
                                             setdiff(colnames(barcode_counts_wider_drug_by_batch), "barcode_id")) |>
  ...
  inner_join(pheno_data |> select(Batch_ID, Pathway, MoA, Compound), 
             by = c("Batch_ID", "Compound")) |> 
  dplyr::distinct() |> 
  tibble::column_to_rownames("original") 

se <- SummarizedExperiment::SummarizedExperiment(
  assays = list(counts = barcode_counts_wider_drug_by_batch |> 
                  tibble::column_to_rownames("barcode_id") |> 
                  as.matrix()),
  rowData = barcode_df,
  colData = barcode_df_drug_by_batch)

saveRDS(se, file = paste0("./results/SummarizedExperiment/se_normalised_per_drug_by_batch_",
                          today_date, ".rds"))

```

```{r}
#| label: SummarizedExperiment-per-compound
#| lst-label: lst-SummarizedExperiment-per-compound
...
barcode_df_drug <- tibble::tibble(Compound = setdiff(colnames(barcode_counts_wider_by_compound), "barcode_id")) |>
...
se <- SummarizedExperiment::SummarizedExperiment(
  assays = list(counts = barcode_counts_wider_by_compound |> 
                  ...
                  as.matrix()),
  rowData = barcode_df,
  colData = barcode_df_drug)

saveRDS(se, file = paste0("./results/SummarizedExperiment/se_normalised_per_compound_",
                          today_date, ".rds"))
```
...
barcode_counts_long <- barcode_counts_normalised |> 
  tidyr::pivot_longer(-barcode_id, 
                      ...
                      values_to = "Barcode_Counts") |> 
  dplyr::inner_join(pheno_data, by="Replicates_ID")# |> 
#   dplyr::select(-Pathway, -MoA, -Samples_ID, 
#                 -Replicates, -Concentrations_ID, -Duration_ID)

barcode_counts_wider_replicates <- barcode_counts_long |> 
  select(-Concentrations, -Duration, -Date, -`Run date`, -Batch_ID, -Compound, 
         -Pathway, -MoA, -Samples_ID, -Replicates, -Concentrations_ID, -Duration_ID) |> 
...
test <- barcode_counts_wider_replicates |>
  dplyr::summarise(n = dplyr::n(), .by = c(barcode_id)) |>
  dplyr::filter(n > 1L)
...
[^1]: Test the **Helmert contrast** to evaluate *drug dose response*, or *time-course replicates*. Also For repeated measures across time, see [One Way repeated measure ANOVA in R](https://www.r-bloggers.com/2025/02/one-way-repeated-measure-anova-in-r/)

### **Derivation of drug fingerprints**:

- [Aggregate by drug and experience.]{fg="blue"} 
...
saveRDS(barcode_counts_discretised_drug, 
        file=paste0("results/fingerprints_compounds_level_", today_date, ".rds"))
```
...
se_normalised <- readRDS("./results/SummarizedExperiment/se_normalised_2025-04-28.rds")
...
- Heatmap at the drug and replicate level are reported in @fig-generate-heatmap-drug.

```{r}
#| label: fig-generate-heatmap-drug
#| fig-cap: "Heamaps at the drug and replicate level."
heatmap_drugs <- ComplexHeatmap::Heatmap(cor_drugs, 
                                         col=col_cor_scale, 
