...
exp281022_expression <- readr::read_csv2("data/barcode-counts/exp281022.csv") |>
  rename(key_barcode = 1)
...
exp281022_expression_timecourse <- readr::read_csv2("data/barcode-counts/exp281022_time course.csv") |>
  rename(key_barcode = 1)
...
controls_exp281022 <- exp281022_expression |> 
  ...
  dplyr::inner_join(exp281022_expression_timecourse |> 
                    ...
                    by="key_barcode")
cor(controls_exp281022$ctl2, controls_exp281022$Ctrl2) # they strongly correlate with each other, but not a perfect mapping
...
Definitely the most challenging to handle with:
  ...
  - `exp200921_dose response osim` has distinct biological objective. 
  
  - [On the other hand, the 4 control biological replicates are shared in both experiences, but **the number of sequenced barcode IDs differ in both experiences, with some only present in `exp200921`, and some others only present in dose response (overall, dose response is the most comprehensive)**.]{fg="red"}
...
pheno_data <- readxl::read_xlsx("data/data-raw/Table of compounds.xlsx",
...
barcodes_IDs <- readxl::read_xlsx("data/data-raw/Table of compounds.xlsx",
                                       sheet="Batch Mapping") |> 
...
[**Please ensure now that the added *Sheet* `Drugs Mapping` in [`Table of compounds_030622`](data/data-raw/Table of compounds.xlsx) is correct.**]{fg="red"}
...
mapping_compounds <- readxl::read_xlsx("data/data-raw/Table of compounds.xlsx",
                                sheet = "Drugs Mapping")
...
flextable::flextable(head(pheno_data_detailed)) |> 
  autofit() |> 
  bold(part="header")
```
...
### Map Results with Single Cell Analyses
...
- Compare with ATC prediction and clustering: [`PDATC-NCPMKL-updated` GH Repo](https://github.com/Lywhere/PDATC-NCPMKL-updated), from 
...
## Store and Publish cell lines

- [`cellosaurus` General Comments](https://www.expasy.org/resources/cellosaurus) and [`cellosaurus` Official Website](https://www.cellosaurus.org/). The paper is associated with @bairoch2018jbt, and @robin2020ijc introduces `CLASTR` that utilises one or more STR (*short-tandem repeat*) profiles to map user-provided cell lines with the ones available in the `Cellosaurus` online resource.
